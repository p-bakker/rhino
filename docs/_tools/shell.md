...
Optimizes at level _optLevel_, which must be `-1` or an integer between `0` and `9`. See [Rhino Optimization](/Optimization) for more details.
...
Load and execute the class named by the string argument _className_. The class must be a class that implements the Script interface, as will any script compiled by [Rhino JavaScript Compiler](/JavaScript_Compiler).
